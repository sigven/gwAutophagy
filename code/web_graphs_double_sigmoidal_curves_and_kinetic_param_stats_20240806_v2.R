...
#Read data
path <- "data-raw"
load(paste0(path,"/Double sigmoidal curve fitting_v2.RData"))
df_DNN_preds <- read_excel(paste0(path,"/Chica_et_al_Data_File_S2_Autophagy_predictions_Statistics_Clustering.xlsx"),
                           sheet=2)
df_DS_curvefits <- read_excel(paste0(path,"/Chica_et_al_Data_File_S2_Autophagy_predictions_Statistics_Clustering.xlsx"),
                              sheet=3)
df_DS_parms <- read_excel(paste0(path,"/Chica_et_al_Data_File_S2_Autophagy_predictions_Statistics_Clustering.xlsx"),
                          sheet=5)
df_DS_parms_ctrs <- read_excel(paste0(path,"/Chica_et_al_Data_File_S2_Autophagy_predictions_Statistics_Clustering.xlsx"),
                               sheet=6)
df_DS_parms_comb <- bind_rows(df_DS_parms,df_DS_parms_ctrs)
...
y_pred <- y_pred %>%
  group_by(Time) %>%
  mutate(d=abs(P1_30_fit-median(P1_30_fit))) %>%
...
ggplot(mat, aes(X, Y))+
  ...
  xlim(x_lower_bound,x_upper_bound)+
  ylim(y_lower_bound,y_upper_bound)+
  labs(x=X, y=Y, color="Reference sets", linetype="Library")+
  scale_fill_jama()+
  scale_color_d3()+
  theme_bw()+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
